...
function csrfProtection({ allowedOriginCheck }) {
  return function csrfMiddleware(req, res, next) {
    ...
    // CSP violation reports are generated by the browser itself, so they can
    // carry neither the CSRF header nor a predictable Origin. The endpoint is
    // write-only, returns 204 regardless of input, and is separately rate
    // limited — there is nothing here for a forged request to accomplish.
    if (req.path === '/api/client-logs/csp') return next();
