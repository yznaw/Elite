...
- If lines are cut off on the right side of the physical paper: the renderer's canvas width (`pos-receipt-renderer.service.ts`'s `widthPx`) must fit the printer's actual **printable** width. For the SRP-QE300, 72mm at 180dpi is approximately 510 dots; round down to **504px (63 whole bytes)** for ESC/POS raster rows. Keep that byte alignment when adapting the renderer to a different printer. Receipt height follows the complete content, including policies and report totals.
- The receipt header (trade name, address, phone, CR number, return policy) is **per branch**, not per tenant — see Settings → General → **Branches**. If a printed receipt shows the wrong shop's address, or blank fields, check two things in order: (1) does the branch this register should belong to actually exist and have its fields filled in, and (2) is this specific register assigned to that branch (Settings → Devices & Security → the Branch column on its row)? An unassigned register silently falls back to the tenant's default branch, which is a different, valid-looking receipt, not an error — it just won't be the shop you're standing in. See `docs/12-pos-system.md` §13.2 for how the fallback resolves.

### Receipt tail appears on the next slip, or printing stops

- Sales, refunds, voids, and Z reports share one raw print job format: initialize the printer, print a 504px-wide `gs_v_0` raster, append the receipt QR where applicable, restore default line spacing, feed six lines, then cut once. The drawer pulse follows the cut for cash operations only; reprints do not open it.
- After installing an update, confirm the POS is running the new build in Hardware. Print a sale, refund, void, Z report, then another sale consecutively on the actual register. Check every slip's final content and intact QR; the Z report must include its refund/void counts and `END OF Z REPORT` before the cut, with no carryover onto the next slip.
- If a job fails, inspect the `PRINT_FAILED` diagnostics and the reported QZ error. A refund or void print failure now offers a persistent **Retry print** action for the saved receipt. Do not repeat the financial transaction to obtain a copy.
- Automated canvas, print-payload, and failure/retry coverage: `cd client && npm run test:pos-printing` (requires Playwright Chromium). These tests mock QZ transport; physical paper feeding, firmware, and Windows queue behavior still require the sequence above.
...
- [ ] **Confirm this register's branch is set up and assigned — do this per shop, not just once.** Superseded from a single tenant-wide profile to per-branch (Settings → General → **Branches**), directly because of the "two shops" launch decision two bullets up. For *each* shop: confirm its branch exists with real trade name/address/phone/CR/return-policy content, then confirm *this specific register* is assigned to that branch (Settings → Devices & Security → the Branch column on its row) — not just that some branch somewhere is filled in. An unassigned register falls back to the tenant's default branch silently: it will print a complete, legitimate-looking receipt, just for the wrong shop. Verify by reading the printed address on a real test receipt from this exact register, not by checking the Settings screen alone.
