...
**Correction, 2026-08-18 — this statement was incomplete.** "`admin-products.route.js` posts `bulk_import` movements" is true of that file's own `/bulk-stock` JSON endpoint, but there are **two** separate bulk-write paths sharing that reason name and the earlier text conflated them: the CSV importer at `admin-bulk-import.route.js` is a different file with its own upsert loop, and it wrote `product_variants.stock_quantity` directly with no ledger row and no parent-product recompute at all. Closed as part of docs/30's F1: the importer now calls the same `previousStockBySku` / `recordMovement` pattern as `replaceVariants()`, and recomputes `products.stock_quantity` from the variant sum before commit — verified against a real database (ledger row present, parent total correct, zero drift). Also added in the same pass: every stock-writing path in this phase (`order-stock.js`, `inventory-ops-service.js`, `admin-products.route.js`, `admin-bulk-import.route.js`) now calls a new `publishStockEvent()` helper (next to `recordMovement` in `inventory-ledger.js`) that pushes a `stock.updated` row onto `pos_events` — the same table `sale-service.js` already used for till-to-till sync. Every connected POS register now sees a web sale, an admin adjustment, a stocktake post, a catalog edit, or a CSV import reflected within its ~1s SSE poll, not just other tills. No frontend change was needed: `pos.component.ts`'s existing listener already reads only `{variantId, stock}` from the event payload.
...
- [x] The CSV bulk importer (`admin-bulk-import.route.js`, distinct from the `/bulk-stock` JSON endpoint above) produces a ledger row with the correct signed delta and recomputes the parent product's total (fixed 2026-08-18).
- [x] Drift query returns zero rows after a mixed order/oversell/reversal/catalog-edit sequence.
- [x] Every stock-writing path (web order, admin adjustment, stocktake, catalog edit, bulk import) publishes a `pos_events` `stock.updated` row that a connected POS register picks up within its ~1s SSE poll (2026-08-18).
