...
- Navigation is an anchor with `routerLink`. Anything that is not navigation (a colour swatch, a size picker, add-to-cart) stays a `<button>`, and must be a **sibling** of the anchor, never nested inside it. The card's `+N` colour control is the one swatch-shaped exception: it only navigates to the product page, so it is an anchor — a sibling of the card's own link, not inside it.
...
The collection card now shares this pattern (see *Colour and size selection*);
the hero's is a button because it navigates in code, the card's is an anchor.
...
**Swatch size and the `+N` link (September 2026).** A card's swatch is a transparent
button (24px, 44px under `pointer: coarse`) wrapping a `.product-color-swatch__dot`
that is all of what is drawn: 14px, 16px on touch. The ring, the border and the
sold-out strike belong to the dot — on the button they would be drawn around the
whole tap target. The dot was previously the button, at 14x14 with 6px between
neighbours, which is under both floors above.

A card shows `MAX_CARD_SWATCHES` (6, or 5 on touch where each target is 44px and the
row must not wrap) and links the rest to the product page with `+N`. The colour on
show is never behind the cap: `visibleColorNames()` moves it into the last visible
slot, so clicking a swatch, or filtering by a colour that sorts late, still shows
...
**Minimum target size.** WCAG 2.5.8 (AA) asks for 24x24 CSS px; touch UIs are held
to Apple's 44. A control that is drawn smaller keeps its drawing and grows its
*target*: either a transparent button around a small inner element (the colour
swatches on both pages), or padding with a matching negative margin (the size
guide link, the breadcrumb, the filter headings). `e2e-storefront/tap-targets.*.spec.ts`
measures both floors, the second in a `pointer: coarse` Playwright project,
because Desktop Chrome never matches that query.
