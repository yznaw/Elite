...
- [ ] **No admin UI screen yet to edit the business profile** — the API exists and is tested via the server test suite, but there's no page in the admin portal for the owner to actually fill in/edit these fields. This is the next concrete step before the receipt can be tested end-to-end with real content.
- [ ] Printed receipt on the actual target printer (Bixolon SRP-QE300) renders correct Arabic text, Qatar local time, and all MOCI-required fields — **not yet tested on real hardware**, only verified to build/compile. QZ Tray's escpos image path is a documented, standard feature, but the actual print output (dithering quality, paper-width fit, dot density) needs a real test print before this can be checked off.
