...
- [ ] Printed receipt on the actual target printer (Bixolon SRP-QE300) renders correct Arabic text, Qatar local time, and all MOCI-required fields — signing is now confirmed working; the actual print output (dithering quality, paper-width fit, dot density, whether Arabic text is legible on the physical receipt) still needs to be visually checked once a full print completes.
