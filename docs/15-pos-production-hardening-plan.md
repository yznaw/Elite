...
- [x] **Admin UI screen added** — Settings → General now has a "Receipt & Legal Profile" card (owner/admin only) wired to `GET`/`PUT /api/pos/business-profile`.
  > **Superseded 2026-08-03**: this single-profile design was replaced by per-branch profiles (`pos_branches`, migration 027) once the business opened a second shop — one row per tenant could not hold two different receipt addresses. The screen is now "Branches" in the same location; `pos_business_profile`/`business-profile-service.js` are gone. Current design: `docs/12-pos-system.md` §13.2.
