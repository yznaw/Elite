...
- [x] **Found and fixed a second real hardware bug, client-side**: after the server-side signing fix, real testing still hit `Signing failed TypeError: l is not a function` thrown inside QZ Tray's own minified code (`_qz.security.callSign`), before any network request. Root cause: QZ Tray's `callCert`/`callSign` helpers branch on `handler.constructor.name === "AsyncFunction"` to decide whether to call the handler directly or wrap it as `new Promise(handler)`. `pos-hardware.service.ts` registered `setCertificatePromise`/`setSignaturePromise` with arrow functions (`() => this.fetchCertificate()`), which are plain `Function`s even though they return a promise — so QZ took the wrong branch and passed an already-settled Promise to `new Promise(...)`, which throws immediately since a Promise instance isn't callable as an executor. Fixed by binding the real `async` class methods directly (`this.fetchCertificate.bind(this)` / `this.fetchSignature.bind(this)`) instead of wrapping them in arrow functions — a bound async method keeps its `AsyncFunction` constructor, so QZ now detects and calls it correctly.
- [ ] Printed receipt on the actual target printer (Bixolon SRP-QE300) renders correct Arabic text, Qatar local time, and all MOCI-required fields — both signing bugs are now fixed and deployed; a full end-to-end print (server signs → QZ prints) has not yet been confirmed to complete and produce readable physical output. This is the next thing to verify on the real register.
