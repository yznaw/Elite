...
| `/restock-requests` | `RestockRequestsComponent` | Authenticated Catalog access. Demand grouped by product/colour/size, request details, status/product/date filters, resend/cancel and CSV export. Linked from Catalog and the sidebar. |
| `/catalog` | `CatalogComponent` | Product grid **and list** view (toggle persisted via `StorageService` — tenant-scoped). Search (matches name, SKU, **and brand**), status quick-filter (All / Active / Hidden / **Out of Stock** (red badge) / **Low Stock** (amber badge)), sort (Name A–Z, Price ↑↓, Stock ↑↓, Newest). The **Low Stock** filter pill is pre-activated via `?stock=low` from the dashboard KPI card. Low-stock threshold from `StoreConfigService.lowStockThreshold()`. **Advanced filter panel**: collection, **brand** (auto-populated from loaded products), **color** (custom swatch picker — hex dot or texture thumbnail per color, from `ref_colors`; supports `?color=X` URL param from reference page usage badge), price range, page size (25/50/100/All). Active filters shown as dismissible chips with color swatch dot. **Bulk Select**: Select All, Set Status, Delete with confirm. **Export CSV** (SKU, Name, Brand, Price, Stock, Status, Variants). **Product drawer** section order (Shopify-style): ① Image Gallery ② Product Info (title EN/AR, brand, SKU) ③ Pricing & Stock ④ Variants ⑤ Description ⑥ Organization (collections + related) ⑦ SEO ⑧ Sync ⑨ Danger Zone. **Variant table** (compact single-row per variant, researched against Shopify / WooCommerce / BigCommerce / Etsy): always-visible columns — Photo · Color · Size · Stock · Price · SKU; collapsible columns (⌄ expand) — Material · Cost · Margin (auto-calculated) · **Size note EN/AR** (a detail this size has and others do not, e.g. a back zipper on the small sizes; shown on the storefront beside the picked size). Stock input turns red when 0 / amber when < 5. Price shows inline "QAR" prefix. SKU is always visible (used for warehouse/POS daily). Color→image linking: click the photo cell in each variant row to open an image picker popover — maps `imageColors[imageUrl] = colorName` so the storefront shows the correct image per color. Image gallery thumbnails show a read-only color badge for linked images. "Generate sizes" wizard. **`ap-save-bar` component**: green sliding bar with Discard / Save changes; appears when the form is dirty, hides when idle. **Arabic Name field** (`nameAr`) stored in `product_translations`. **Cost price per variant** (`cost_price_cents`) with real-time **margin formula** (color-coded pill: green ≥ 40 %, amber 20–40 %, red < 20 %). **Stock is auto-computed** from variant sum when variants exist. **SEO fields** (`meta_title`, `meta_desc` 160-char counter, slug). **Duplicate Product**. **Bulk Import** + **Stock Update mode** (Dry-Run, Retry Failed, Import History). **Description section** is split into three purpose-named fields: **Hook** (`shortEn`/`shortAr`, plain text, ~90 chars) seeds the home hero tagline when a product is linked to a slide; **Short description** (`teaserEn`/`teaserAr`, plain text, ~160 chars) shows directly under the product name on the storefront product page; **Material & Care** (`careEn`/`careAr`, rich text, stored in the `care_instructions` JSONB column) fills the Material & Care section on the storefront product page, falling back to the legacy `enDesc`/`arDesc` long description for products saved before this split existed. **SKU renaming** when a color group is renamed now also updates the SKU segment in all affected variants automatically. **Variant lifecycle (2026-09-15):** *Add variant* and *Add size* open the target colour group, scroll the new row into view and focus it. Picking a size that already exists in the same colour, or renaming a colour group onto a colour that already has one of its sizes, is refused with a toast. Removing a saved row or a row with stock asks for confirmation (rows added this session with no stock go immediately); removing the last size of a colour also clears that colour's photo links, SKU draft and open state. Save points at the first row with a missing or duplicate SKU. **Save baseline:** `save()` snapshots the form before the request and, on success, sets `initial` from the server response (`makeFormFromProduct(saved)`). If the form changed while the request was in flight, those edits stay dirty and the save bar stays up; otherwise the form is replaced with the saved product (real variant ids, barcode defaulting to SKU). A size that was counted in a stocktake is hidden (`is_active = false`) by the server instead of deleted, and no longer loads in the drawer. **Catalog + drawer audit (2026-09-15):** the drawer receives `drawerProducts()` (the filtered list plus the open product), so a save that moves a product out of the current filter, page or sort keeps the drawer on that product; `safePage()` clamps the page when filters, page size or deletes shrink the list; *New product* resets the sort to Newest. `save()` resolves its target product once, checks name/brand/SKU and the slug before sending, sends `expectedStock` (the stock it loaded), and on `STOCK_CHANGED` reloads current stock into the form and baseline so the next save does not undo a sale; collection sync runs last with its own error toast. Image uploads only update the product they were started on and keep unsaved gallery edits; delete captures its product before the 600 ms delay and blocks prev/next meanwhile. Prices are whole QAR (inputs round). SEO title, description and slug stay empty unless typed, with the generated text shown as placeholders. A localStorage draft is only restored when it was made against the same loaded data. Single and bulk delete archive; *Undo* calls `POST /api/admin/products/:id/restore` before re-inserting the row, and a failed delete leaves the row in place. Bulk actions skip unsaved `P-NEW-` rows and roll back to a snapshot on failure. The empty state is not shown while loading, a failed load toasts, the Out of Stock badge uses the same rule as its filter, and CSV export escapes every field and guards spreadsheet formulas. Leaving with unsaved product edits is guarded: the drawer listens to `window:beforeunload`, and the `/catalog` route has a `canDeactivate` guard that calls `CatalogComponent.canLeave()` → `ProductDrawerComponent.confirmLeaveIfDirty()` (confirm dialog). The per-colour *Generate sizes* chart choice is a signal (`groupSizeSet`), so the Generate button enables as soon as a chart is picked. **New variant flow:** the footer *Add variant* creates its own highlighted group (`__new__`, keyed by `pendingVariantIds`) at the top of the list, separate from any older colourless rows, with the colour select focused and the hint "Choose a colour, then add its sizes". A second click brings that group back instead of adding another blank. Choosing a colour moves it into that colour's group, opened at the new row, where sizes are added one at a time with *+ Size* (*Generate sizes* stays optional and replaces the blank row). Every variant row needs a size: save is refused and the row highlighted when one is missing, and the API returns 422 `Variant <sku> needs a size.` Covered by `client/e2e-auth/product-variants.spec.ts`. |
...
| `/orders` | `OrdersComponent` | Searchable order table, payment/fulfillment filters, **Date Range filter** (All Time / Today / This Week / This Month / Custom — custom shows `date from/to` inputs). Active date range displayed as a dismissible chip. `clearFilters()` resets date range along with other filters. **CSV export** of the current filtered set (UTF-8 BOM). **Production hardening (2026-06):** skeleton table/card loaders while `loading()` is true; `loadError` signal shows a red error banner with Retry button on initial load failure (silent background refresh swallows errors); 300ms RxJS search debounce via `Subject` + `takeUntil` teardown; 15s background polling syncs the list while the drawer is open; `OrderDrawerComponent` now routes all `PATCH /status` calls through `safeUpdateStatus()` which re-fetches the order from the server on API error to resync local state. Full-height drawer with status workflow stepper, tracking number, internal notes & timeline, **Print Invoice** button. **Invoice hardening (2026-09):** renders into a hidden same-page `<iframe>` and calls `print()`, so pop-up and ad blockers no longer kill it (the old `window.open` + `document.write` was silently blocked); honours `lang`/`dir` from `LocaleService` so the Arabic invoice lays out RTL; and prints a **Delivery** block (carrier, service, tracking number, shipped/delivered dates, carrier ETA) from `order.delivery`, omitted entirely when no shipment exists. No PDF library is used: the browser's own print engine is the only one here that shapes Arabic correctly, and the button was already labelled "Print / Save as PDF". **Correctness (2026-09):** `seedTimeline()` — which fabricated history containing hardcoded `2026-04-27` / `2026-04-29` dates whenever the timeline was empty — is deleted; an undefined `timeline` renders a skeleton until `GET /admin/orders/:id` resolves, and a failed detail fetch raises a retryable toast instead of being swallowed. Column sorting and CSV export are both server-side over the full result set. The list's **Mark Fulfilled** now requires a tracking number and a confirm dialog, matching the drawer's own rule. `?id=` deep links fetch by id instead of searching only the loaded page. |
| `/customers` | `CustomersComponent` | Customer table/cards view (toggle persisted), **Add Customer** create flow (synthesises a draft, discards on close without save), fully editable detail drawer with real linked-orders history (rows navigate to `/orders?id=…`). **Production hardening (2026-06):** skeleton loaders while `loading()` is true; `loadError` banner with Retry; 300ms debounced search; **Export CSV** wired (was previously a no-op button); EU size pill hidden when `sizePref` is `0` or `null`. Customer drawer fetches orders from `GET /admin/customers/:id/orders` (matches by `customer_id` OR `customer_email`), 30s polling with silent refresh + "Updated HH:MM" timestamp, `phone` field, async save that only shows "Saved" after API response and reverts to dirty on error, soft-delete that preserves order history. **Correctness (2026-09):** the parent's duplicate `create()`/`update()` call is removed — saving previously fired two HTTP requests plus a stray `PATCH` against a just-created id; the delete success toast now fires only after the server confirms, keeps the row on failure, and offers **Undo** via the previously-dead `restore()` endpoint. The list is server-paginated, server-searched and server-sorted (`GET /admin/customers` returns `{ customers, total, page, limit, pages }`); CSV export walks every page rather than dumping the current one; `?highlight=` deep links from an order now actually resolve. Both drawers are `role="dialog" aria-modal="true"` and close on Escape (the customer drawer shakes the save bar instead when dirty). |
| `/analytics` | `AnalyticsComponent` | Revenue chart, traffic sources, conversion funnel, per-product cost/margin table, a **Shipping Costs by Product** report (`GET /admin/analytics/shipping-costs`) listing every active product with its shipping-cost coverage, missing ones first, with search and an "only incomplete" filter, and an **Expenses & Net Profit** section (`GET /admin/analytics/profit-summary`) showing revenue - COGS - operating expenses as a waterfall plus an expenses-by-category donut. COGS here is computed from what actually sold, not averaged catalogue margin; a warning shows when sold line items have no cost recorded. |
| `/expenses` | `ExpensesComponent` + `ExpenseDrawerComponent` | **Operating expense ledger.** Date-range/category/search filters, totals strip (grand total + top 3 categories), ledger table with category pills and per-row receipt indicator, CSV export, and POS cash-out import. Drawer handles amount/date/category/vendor/payment method/note/receipt upload and a `none/monthly/yearly` recurrence. Recurring bills are stored once and projected forward on read, shown with an "Upcoming" pill; editing a projected occurrence materialises just that period. Creating closes the drawer so a second Save cannot file a duplicate. Owner/admin only. |
| `/settings` | `SettingsComponent` | **General tab:** Store info (name, currency, timezone, language — `PATCH /api/admin/settings/store`) + **Low Stock Threshold** number input — sets `StoreConfigService.lowStockThreshold()`, persisted tenant-scoped via `StorageService`, consumed by catalog and dashboard. **Team tab:** team members (list, role change, status toggle, **branch** — `GET/PATCH /api/admin/settings/team/:id`). The Branch column appears once the tenant has more than one branch and sets `admin_users.pos_branch_id`, which scopes the POS "which till is this?" picker to that branch's registers; "All branches" clears it and is the right default for owners and admins. See [12 – POS System](./12-pos-system.md). **Team Invitations** — invite by email + role (`POST /api/admin/settings/invitations`), shows generated invite link in a copy-able input, lists pending invitations with revoke button. **Integrations tab.** Owner/admin only. |
...
- **Methods:** `list(params?)` → `{ customers, total, page, limit, pages }` (params: `page`, `limit`, `q`, `sort`, `dir`), `get(id)`, `getOrders(id)` → `Order[]`, `create(payload)`, `update(id, payload)`, `remove(id)` (soft-delete), `restore(id)`
...
- **401 handling:** Uses `toast.error` (not warning) and redirects to `/login?returnUrl=<current-path>` so the admin lands back on the same page after re-authenticating. Skipped when the request is the `/auth/me` auth probe, a manager-PIN verify, a POS register-binding call (`/pos/registers/(check-in|claim|enroll|release)`), when already on `/login`, or when a redirect already fired in the last 10 seconds.
- **Login-bounce loop breaker:** redirecting to `/login` only helps when signing in changes the outcome. `LoginComponent.ngOnInit` calls `auth.me()` and, finding a perfectly valid session, sends the operator straight back to `returnUrl` — so a 401 that was *not* an expired login ping-ponged between `/pos` and `/login` forever, one "Session expired" toast per lap, escapable only by clearing cookies. That was a real production incident: a POS terminal whose register had been deleted got `401 REGISTER_CREDENTIAL_INVALID` from `/pos/registers/check-in`. Both halves are fixed — the server no longer answers 401 there (see [12 – POS System](./12-pos-system.md)), and the interceptor caps `/login` redirects at one per 10 seconds so any future misclassified 401 costs a single bounce instead of an endless one.
...
| `analytics.*` | Analytics page (includes `analytics.profit.*` for the net-profit section) |
| `expenses.*` | Expenses ledger, categories, payment methods, recurrence |
...
| `Order` | id, date, customer, total, payment, fulfillment, items[], trackingNumber?, delivery?, timeline[]?, notes[]? | Orders |
| `OrderDelivery` | carrier?, service?, trackingNumber?, trackingUrl?, shippedAt?, deliveredAt?, eta? | Orders · invoice |
...
- **Label printing:** `LabelPrinterService` (`services/label-printer.service.ts`) renders Code128 barcodes via `jsbarcode` into a print-ready popup window (same `window.open` + `window.print()` pattern as the order invoice), showing brand, product name, variant, barcode, and price. Triggered per-variant ("Print barcode label" icon) or in bulk for the whole product ("Print Labels" in the variants footer).
  - **Price in both languages, one row.** The label's price line is a two-column flex row: `QAR 1299.50` on the left, the same number read in Arabic on the right (`١٬٢٩٩٫٥٠ ر.ق`). The Arabic form comes from `Intl.NumberFormat('ar-QA-u-nu-arab', …)` — the `-u-nu-arab` extension is what asks for Arabic-Indic digits, rather than transliterating them by hand — exported as `arabicPrice()` so the on-page sheet and the printed sticker cannot drift. The Arabic column carries its own `direction: rtl` and a font stack with real Arabic glyphs, because Helvetica alone renders these as boxes on a Windows till.
  - **On-page barcode sheet.** "Show barcodes" in the variants footer renders every variant's code on the page — the same JsBarcode output the printer gets, laid out in the same order (brand, name, variant, bars, code, both prices). Barcodes previously existed only inside the print popup, so checking a code meant printing a label and looking at the paper; a code can now be scan-tested straight off the screen. `ap-barcode` (`shared/barcode/barcode.component.ts`) is the shared renderer, also used live under the barcode field in each variant's expanded row so a typo is visible where it is made.
...
### Restock demand (2026-09-14)

`RestockRequestsService` uses `/api/admin/restock-requests`, `/summary`, `/:id/resend`, `/:id/cancel`, and `/export.csv`. Requests are tenant-scoped and use the same session access as Catalog. Detail lists use pages of 100; grouped summaries return up to 1,000 combinations. Resend schedules a due request; it still waits for stock and valid email configuration. Cancelled requests cannot be reactivated by resend.

The Catalog editor reads waiting counts beside each variant. The dashboard reads the five most-requested sold-out combinations. The underlying table is `restock_notifications` (migration 039); all page text is English/Arabic. See [the stock plan](./35-stock-availability-and-restock-plan.md) for dispatch, retention and deployment details.
