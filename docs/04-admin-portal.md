...
| `/storefront` | `StorefrontComponent` | Section editor with drag & drop, draft → publish, preview. **Featured Collections panel:** searchable collection picker chips, manual handle entry (`/collection/{handle}`), collection IDs persisted in `home-collections` block settings JSONB and restored on draft load. |
| `/home-content` | `HomeContentComponent` | Hero + Featured Collections tile editor. **Each tile** has a **Linked Collection dropdown** (auto-fills title, `/collection/{handle}` link, and cover image from the selected system collection; all fields remain editable). All image pickers (hero, tiles, story hero, story chapters) have **Upload** + **Pick from Media** buttons — the media picker is a slide-in panel loaded lazily and shared across all image slots. |
