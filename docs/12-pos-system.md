...
**The picker and every register-bound request are scoped to the staff member's branch.** `admin_users.pos_branch_id` (migration 035) says which branch a person works at; `listSelectableRegisters` filters to it, `claimRegister` refuses an out-of-branch till, and the central POS device guard re-checks the current database assignment on every operation with `403 REGISTER_OUT_OF_BRANCH`. `NULL` means unscoped, which is the right default for owners and admins who move between counters. A register with no branch of its own is matched against the tenant's default branch, the same one that prints on its receipts. User and register branch changes are blocked while the affected register/user has an active shift.

**Claiming rotates both the credential and device lease.** Migration 036 gives every register a random `device_lease_id`; the signed device cookie carries the register and lease together. The central POS guard validates that lease on every post-enrollment route, so the previous machine fails its very next read or write with `409 REGISTER_LEASE_INVALID`, not merely its next check-in. Check-in also rotates both values, preventing a copied/stale IndexedDB credential from repeatedly reclaiming the lease.

**Taking over any already-paired till is explicit.** A different browser receives `409 REGISTER_TAKEOVER_CONFIRM`, including when both browsers use the same account or no shift is open. The client resends with `confirmTakeover: true` plus a manager PIN. A shop with no manager PIN configured still needs explicit confirmation, but cannot provide a PIN it has never configured.
...
Clearing the browser profile removes the local credential and requires an explicit takeover of the same active register from the picker (manager PIN when configured). For a settled or revoked physical device, **Replace / re-pair** in Registered Devices issues a 15-minute, one-use replacement token while preserving the logical register id and history. Revoke only after the shift is closed and known pending/rejected offline counts are zero; the server enforces both guards.

**The device binding is a cookie, not a login session.** A long-lived, httpOnly, HMAC-signed `elite.pos_device` cookie carries the register id and current lease id and is preferred over the login-session copy. `requireRegister()` verifies tenant, status, lease and current user branch on every operation.
...
**`POST /registers/release` is the escape hatch.** It drops the session binding and expires the device cookie. Manual reset is blocked while a local shift or unresolved offline sale exists. A safe reset clears the old receipt reservation before another till can be selected; server rejection still releases the dead binding while retaining unresolved queue records for support/recovery.
...
Elite allocates receipt numbers in tenant-wide blocks of 100. Blocks cannot overlap and are tied to one register. The browser persists `registerId` with the current block and refuses to reuse the block after switching registers; pre-migration cached blocks without that identity are discarded and replaced online.
